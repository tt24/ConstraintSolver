...
public class Solver {
	...
	public boolean checkConsistency(ArrayList<DecisionVariable> variables, ArrayList<Constraint> constraints, int depth,
			DecisionVariable var, HashMap<DecisionVariable, TreeSet<Integer>> savedDomains, ArrayList<DecisionVariable> orderedVariables) {
		...
		for (int future = depth + 1; future < variables.size(); future++) {
			...
			for (int future = 0; future < orderedVariables.size(); future++) {
				for (Constraint constraint : constraints) {
					if (constraint.containsVar(variables.get(future)) && constraint.containsVar(var)) {
						consistent = revise(constraints, constraint, savedDomains, null);
						if (!consistent) {
	...
	public boolean forwardChecking(ArrayList<DecisionVariable> variables, ArrayList<Constraint> constraints, int depth,
			ArrayList<DecisionVariable> orderedVariables) {
		HashMap<DecisionVariable, TreeSet<Integer>> savedDomains = new HashMap<>();
		DecisionVariable var;
		if (orderedVariables == null) {
			var = variables.get(depth);
		} else {
			Iterator<DecisionVariable> iterator = orderedVariables.iterator();
			while(iterator.hasNext()) {
				DecisionVariable dVar = iterator.next();
				System.out.println(dVar.getName()+ " domain size "+ dVar.getDomain().size());
			}
			Collections.sort(orderedVariables);
			System.out.println(depth);
			var = orderedVariables.get(0);
			if(var.getDomain().size()==0) {
				return false;
			}
			orderedVariables.remove(0);
		}
		Iterator<Integer> iterator = var.getDomain().iterator();
		while (iterator.hasNext()) {
			int value = iterator.next();
			var.setAssignedValue(value);
			System.out.println("Assigning " + value + " to " + var.getName());
			if (checkConsistency(variables, constraints, depth, var, savedDomains, orderedVariables)) {
				if (depth == variables.size() - 1) {
					return true;
				} else {
					if (forwardChecking(variables, constraints, depth + 1, orderedVariables)) {
						return true;
					}
				}
			}
	...
	public static void main(String[] args) {
		...
		if (solver.ac3(variables, constraints, unaryConstraints)) {
			solver.forwardChecking(variables, constraints, 0, null);
